        ...
        def on_picture(_, filepath):
            ...
            try:
                ...
                if new_filepath.exists():
                    print(f"[Camera] Removing existing {new_filepath}", flush=True)
                    new_filepath.unlink()
                
                # Rename the file to document.jpg
                print(f"[Camera] Attempting to rename {original_path} to {new_filepath}", flush=True)
                original_path.rename(new_filepath)
                Logger.info(f"CameraScanScreen: Photo renamed to -> {new_filepath}")
                print(f"[Camera] Successfully renamed to -> {new_filepath}", flush=True)
                final_path = new_filepath
            except Exception as e:
                Logger.warning(f"CameraScanScreen: Failed to rename photo: {e}")
                print(f"[Camera] Rename failed: {e}, keeping original: {original_path}", flush=True)
